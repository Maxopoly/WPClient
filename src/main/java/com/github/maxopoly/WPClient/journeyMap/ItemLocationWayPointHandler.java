	...
	public synchronized void markLocations(WPItem item, Map<Location, Integer> amount) {
		...
		if (amount.isEmpty() && mc.thePlayer != null) {
			mc.thePlayer.addChatMessage(new TextComponentString(
					String.format("%s[WPC]  %sError:  %sCouldn't find any %s%s%s.",
						TextFormatting.BLUE, TextFormatting.RED, TextFormatting.GRAY, TextFormatting.WHITE,
						item.getPrettyName(), TextFormatting.GRAY)));
			return;
		}
		int sum = 0;
		for (Entry<Location, Integer> entry : amount.entrySet()) {
			createWaypoint(entry.getKey(), item.getPrettyName(), item.prettifyItemCountWaypointName(entry.getValue()));
			sum += entry.getValue();
		}
